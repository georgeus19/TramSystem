...
public class MissionPlanningService {
	...
	public async Task<IEnumerable<MissionPlanning.Api.Mission>?> GetMissions() {
		IEnumerable<MissionDto>? missions =  await _httpClient.GetFromJsonAsync<IEnumerable<MissionDto>>("http://localhost:5001/api/missions");
		return missions?.Select(m => m.ToMission());
	}

	public async Task<MissionPlanning.Api.Mission?> PlanMission() {
		var response = await _httpClient.PostAsync("http://localhost:5001/api/missions", null);
